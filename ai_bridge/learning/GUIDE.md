...
Manager가 자동으로 작업 유형을 판단하고 다른 기준 적용:

| 유형 | 키워드 | 판단 기준 |
|------|--------|------------|
| 코드 | (기본) | flutter analyze 에러 0개, 코드 품질 |
| 분석 | 분석, 파악, 검토 | 분석 충실성, 거짓 보고 여부 |
| 커밋 | commit, push, git | 커밋/푸시 성공 여부 |
| 정리 | 삭제, 정리, cleanup | 파일 삭제 여부, 실수로 중요 파일 삭제 안 함 |

**예시**:
```
빅스텝: "Phase 2 현재 상태 분석해"
→ Manager: "분석" 키워드 감지 → 분석 기준 적용
→ flutter analyze 안 돌리고 분석 충실성으로 판단
```
