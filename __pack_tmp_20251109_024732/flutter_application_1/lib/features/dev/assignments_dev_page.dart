...
class _AssignmentsDevPageState extends State<AssignmentsDevPage> {
  ...
  Future<void> _bootstrap() async {
    try {
      ...
      _groups = await _readGroupsFromIdToken();
      setState(() {});
      _appendLog('[Auth] current user: $_username, groups=$_groups');
    } catch (e) {
      _appendLog('[Auth] getCurrentUser error: $e');
    }
  }

  Future<List<String>> _readGroupsFromIdToken() async {
    try {
      final session = await Amplify.Auth.fetchAuthSession() as CognitoAuthSession;

      // v2.6.x: tokens는 *Result* 래퍼로 제공됨
      final tokensResult = session.userPoolTokensResult;
      if (!tokensResult.isSuccess) return [];
      ...
      final groups = (map['cognito:groups'] as List?)?.cast<String>() ?? [];
      return groups;
    } catch (_) {
      return [];
    }
  ...
  bool get isTeacher => _groups.contains('teachers') || _groups.contains('owners');
  bool get isStudent => _groups.contains('students') || _groups.contains('owners');
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                _Header(username: _username, groups: _groups),
...
class _Header extends StatelessWidget {
  const _Header({required this.username, required this.groups});
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Wrap(
          spacing: 6,
          children: groups.map((g) => Chip(label: Text(g))).toList(),
        ),
      ],
    );
  }
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'studentUsername (예: student1)',
                    ),
                  ),
                ),
                const SizedBox(width: 12),
